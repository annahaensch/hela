...
def get_true_ll(gen, train_data, test_data):
    ...
    train_log_prob = true_hmm_inf.predict_hidden_state_log_probability(train_data)
    test_log_prob = true_hmm_inf.predict_hidden_state_log_probability(test_data)

    true_train_ll = (-1 / train_data.shape[0]) * logsumexp(true_hmm_inf._compute_forward_probabilities(train_log_prob)[-1])
    true_test_ll = (-1 / test_data.shape[0]) * logsumexp(true_hmm_inf._compute_forward_probabilities(test_log_prob)[-1])
...
def train_hmm_em(gen, random_state, train_data):
    ...
    observations =  [{'name':col,'type':'continuous','dist':'gaussian','dims':1} for col in list(
        gen.gaussian_values.columns)]


    hmm_spec = {
        'hidden_state': {'type': 'Finite',
    ...
    untrained_hmm_model = hmm_model_config.to_model(set_random_state = random_state)

    em_alg = hmm.LearningAlgorithm()

    em_hmm_model = em_alg.run(untrained_hmm_model, 
                    train_data, 
                    n_em_iterations = ITERATIONS)
...
def train_fhmm_vi(gen, random_state, train_data, train_fact_hidden_states):
    ...
    """
    fhmm_training_spec = hmm_gen.data_to_fhmm_training_spec(train_fact_hidden_states, 
...
def compute_model_ll(model, train_data, test_data, model_type):
    ...
    train_ll = (-1/train_data.shape[0]) * logsumexp(hmm_inf._compute_forward_probabilities(train_log_prob)[-1])
    test_ll = (-1/test_data.shape[0])  * logsumexp(hmm_inf._compute_forward_probabilities(test_log_prob)[-1])
...
def main(argv):
    ...
    time_df = pd.DataFrame(columns = ['systems','states','random','trial','em','vi','gibbs'])
    ll_df = pd.DataFrame(columns = ['systems','states','trial','random','train_true','test_true','train_em','test_em','train_vi','test_vi','train_gibbs','test_gibbs'])
    ...
    for t in range(len(RANDOM_STATES)):
        ...
        train_fact_hidden_states = gen.generate_hidden_state_sequence(n_observations = 400)
        test_fact_hidden_states = gen.generate_hidden_state_sequence(n_observations = 400)

        vec_list = list(itertools.product(*[[t for t in range(n)] for n in gen.ns_hidden_states]))

        flattened_state_dict = {i:vec_list[i] for i in range(len(vec_list))}
        flattened_state_dict_inv = {str(list(v)) : k for k,v in flattened_state_dict.items()}

        train_flat_hidden_states = pd.Series([flattened_state_dict_inv[str(list(v))] for v in train_fact_hidden_states.values], 
                              index = train_fact_hidden_states.index)
        test_flat_hidden_states = pd.Series([flattened_state_dict_inv[str(list(v))] for v in test_fact_hidden_states.values], 
        ...
        em_train_ll, em_test_ll = compute_model_ll(em_hmm_model, train_data, test_data, "hmm")
        ...
        vi_start = time.time()
        vi_alg, vi_fhmm_model = train_fhmm_vi(gen,random_state, train_data, train_fact_hidden_states)
        ...
        vi_train_ll, vi_test_ll = compute_model_ll(vi_fhmm_model, train_data, test_data, "fhmm")
        ...
        gibbs_alg, gibbs_fhmm_model = train_fhmm_gibbs(gen, random_state, train_data, train_fact_hidden_states)
        ...
        gibbs_train_ll, gibbs_test_ll = compute_model_ll(gibbs_fhmm_model, train_data, test_data, "fhmm")
    ...
    time_df.to_parquet("training_table_data/time_df_{}_{}.pq".format(n,m))
    ll_df.to_parquet("training_table_data/ll_df_{}_{}.pq".format(n,m))
    em_df.to_parquet("training_table_data/em_df_{}_{}.pq".format(n,m))
    vi_df.to_parquet("training_table_data/vi_df_{}_{}.pq".format(n,m))
    gibbs_df.to_parquet("training_table_data/gibbs_df_{}_{}.pq".format(n,m))
